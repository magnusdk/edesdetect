...
class TrainingState(NamedTuple):
    ...
    target_params: networks_lib.Params
    target_state: networks_lib.Params
...
class SGDLearner(acme.Learner):
    ...
    def __init__(
        ...
        def sgd_step(
            ...
            (loss, (new_state, extra)), grads = jax.value_and_grad(
            ...
            new_target_state = rlax.periodic_update(
                new_state, state.target_state, steps, target_update_period
            )
        ...
        initial_params, initial_state = self.network.init(key_params)
        initial_target_params, initial_target_state = self.network.init(key_target)
        ...
        def update_priorities(reverb_update: ReverbUpdate) -> None:
            ...
            keys, priorities = tree.map_structure(
                utils.fetch_devicearray, (reverb_update.keys, reverb_update.priorities)
            )
    ...
    def get_variables(self, names: List[str]) -> List[networks_lib.Params]:
        return [self._state.params, self._state.state]
