...
class PrioritizedDoubleQLearning(learning_lib.LossFn):
    ...
    def __call__(
        ...
        key_tm1, key_t_value, key_t_selector = jax.random.split(key, 3)
        transitions: types.Transition = batch.data
        keys, probs, *_ = batch.info

        # Forward pass.
        q_tm1, state_tm1 = network.apply(
            params, state, key_tm1, transitions.observation
        )
        q_t_value, state_t_value = network.apply(
            target_params, target_state, key_t_value, transitions.next_observation
        )
        q_t_selector, state_t_selector = network.apply(
            params, state, key_t_selector, transitions.next_observation
        )
