...
** The Cardiac Cycle
 ...
 A cardiac cycle consists of multiple events as visualized in the Wiggers diagram (figure [[fig:wiggers_diagram]]). ......
...
Three values are tested for the exploration hyper-parameter $\epsilon$: $\epsilon=0.0$, $\epsilon=0.01$, and $\epsilon=0.1$. The assumption is that using $\epsilon=0.0$ is best because the reward signal is so noisy from the beginning because the reward is carried from the end of an episode an back to all steps. Using an $\epsilon>0.0$ may add even more noise as it becomes harder for the network to distinguish whether it was its own actions or the randomness of exploration that caused a drop in returns.

The hyper parameters listed in table [[tbl:gaafd_hyper_params]] are used unless otherwise specified.
